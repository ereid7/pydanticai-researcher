...
agent = Agent(
    ...
    2. Use the search_web and crawl_additional_site tools to find additional relevant information when needed, such as wikipedia
...
def get_page_content(page_number: int, total_pages: int) -> str:
    ...
    if not (0 <= page_number < len(crawled_pages)):
        return (
            f"Invalid page number. Please choose between 0 and {len(crawled_pages)-1}"
        )
...
async def main(url: str):
    ...
    2. Use search_web to find additional relevant information about the company, wikipedia, etc.
    3. If you find particularly relevant websites, use crawl_additional_site to analyze them in depth

    Write a sales email to the company, based on the product we are trying to sell them. Our product is firstbite.io, you will need to search and crawl this.
    """
    ...
    print("\nReport:")
    print("=======")
    print(f"\Sales Email:\n{result.data.sales_email}")
    print(f"\Email Subject:\n{result.data.email_subject}")
    print(f"\Email Content:\n{result.data.email_content}")
    # print("\nKey Points:")
